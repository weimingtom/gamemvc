...
public:
	...
	bool isTouching( 	Vector2D pos,
		...
		return Vec2DDistanceSq( m_vPos, pos ) < ( EntityRadius + m_dRadius )
...
public:
	...
	bool isTouching( 	Vector2D pos,
						double EntityRadius ) const {
		InvertedAABBox2D Box( Vector2D( pos.x - EntityRadius, pos.y
				- EntityRadius ), Vector2D( pos.x + EntityRadius, pos.y
				+ EntityRadius ) );

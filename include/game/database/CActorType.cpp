...
bool CActorType::Load( TiXmlElement* pXMLData )
{
  ...
  THROW_GAME_EXCEPTION_IF( !(pXMLData->Attribute("script")),
                           "Error m_LuaScript ActorType no definido" );
  ...
  while ( pPersonality ) {
    ...
    THROW_GAME_EXCEPTION_IF( !pPersonality->Attribute("name"),
                             "Error Name Personality no definido" );
...
CPersonality* CActorType::GetPersonality( const std::string& personality )
{
  ...
  THROW_GAME_EXCEPTION_IF( !PersonalityExist(personality),
                           "Error GetPersonality personality no definido " );

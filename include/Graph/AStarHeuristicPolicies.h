...
//-----------------------------------------------------------------------------
//the Manhattan distance. For 8 directions.
//	Amit Patel’s A*'s Use of the Heuristic
//-----------------------------------------------------------------------------
...
public:
  ...
  static double Calculate(const graph_type& G, int nd1, int nd2)
  {
	Vector2D p = G.GetNode(nd1).Pos()-G.GetNode(nd2).Pos();
	double D 	= 1.0;
	double D2	= D* sqrt(2.0);
	double h_diagonal = std::min(abs(p.x), abs(p.y));
	double h_straight = abs(p.x) + abs(p.y);
	return D2 * h_diagonal + D * (h_straight - 2.0*h_diagonal);
  }
};
#endif

...
void RegisterToLuaBaseGameEntity(lua_State* Lua){
	...
	[
		 ...
		 .property("m_vPos",&BaseGameEntity::Pos,&BaseGameEntity::SetPos)
		 .property("m_EntityType",&BaseGameEntity::EntityType,&BaseGameEntity::SetEntityType)
	];
}
void RegisterToLuaMovingEntity(lua_State* Lua){
	module(Lua)
	[
		 class_<MovingEntity, bases<BaseGameEntity> > ("MovingEntity")
		 /*
...
void RegisterToLuaGameWorld(lua_State* Lua){
	module (Lua)
	[
		 class_<PlayModel, bases<Model> >("PlayModel")
...
void RegisterToLuaGoal(lua_State* Lua){
	...
	module(Lua)
	[
	 	 class_< Goal <CActorMapa>  >("Goal")
	 	 .property("m_pOwner",&Goal <CActorMapa>::GetOwner)
	];

	module(Lua)
	[
	 	 class_<Goal_Composite <CActorMapa>, bases<Goal <CActorMapa> > >("Goal_Composite")
	 	 .def("RemoveAllSubgoals",&Goal_Composite <CActorMapa>::RemoveAllSubgoals)
		 .def("AddSubgoal",&Goal_Think::AddSubgoal, adopt(_2)) // Con esto dejamos que C++ maneje el destructor del
	...
	module(Lua)
	[
		 class_<Goal_Think, bases < Goal_Composite<CActorMapa> >, Goal_Think_wrapper >("Goal_Think")
		 .def(constructor<CActorMapa*>())
		 .def("Arbitrate",&Goal_Think::Arbitrate,&Goal_Think_wrapper::default_Arbitrate)

	];

	module(Lua)
	[
		 class_<Goal_Wander, bases <Goal<CActorMapa> > ,Goal_Wander_wrapper>("Goal_Wander")
		 .def(constructor<CActorMapa*>())
	];
	module(Lua)
	[
		 class_<Goal_MoveToPosition, bases <Goal_Composite<CActorMapa> > >("Goal_MoveToPosition")
...
void RegisterToLuaActorMapa(lua_State* Lua){
	module(Lua)
	[
		 class_< CActorMapa, bases< MovingEntity >, ActorMapa_wrapper > ("CActorMapa")
		 .enum_("message_type")
		 [
			  value("Msg_PathReady",CActorMapa::Msg_PathReady),
			  value("Msg_NoPathAvailable",CActorMapa::Msg_NoPathAvailable),
			  value("Msg_Goto",CActorMapa::Msg_Goto)
		 ]
		 .def(constructor<PlayModel*,Vector2D,CActorType*> ())
		 .def("World",&CActorMapa::World)
		 .def("HandleMessage",&CActorMapa::HandleMessage,&ActorMapa_wrapper::default_HandleMessage)
		 .def("UpdateFrame",&CActorMapa::UpdateFrame,&ActorMapa_wrapper::default_UpdateFrame)
		 .property("m_pBrain",&CActorMapa::GetBrain,&CActorMapa::SetBrain,luabind::detail::null_type(),adopt(_2))
		 .def("SmoothingOn",&CActorMapa::SmoothingOn)
		 .def("isHeadingAt",&CActorMapa::isHeadingAt)
		 .property("m_Personality",&CActorMapa::GetPersonality,&CActorMapa::SetPersonality)
		 .property("m_State",&CActorMapa::GetState,&CActorMapa::SetState)
		 .property("m_Dir",&CActorMapa::GetDir,&CActorMapa::SetDir)
		 .property("m_iFrame", &CActorMapa::GetFrame, &CActorMapa::SetFrame)
		 .property("m_pSteering",&CActorMapa::Steering)
...
void RegisterToLuaCActorType(lua_State* Lua){
	module(Lua)
	[
		 class_<CActorType> ("CActorType")
	];
}
void RegisterToLuaSteeringBehavior(lua_State* Lua){
	module(Lua)
	[
		 class_<SteeringBehavior>("SteeringBehavior")
		 .def("SeekOn",&SteeringBehavior::SeekOn)
		 .def("SeekOff",&SteeringBehavior::SeekOff)
		 .def("ArriveOn",&SteeringBehavior::ArriveOn)
		 .def("ArriveOff",&SteeringBehavior::ArriveOff)
		 .def("SeparationOn",&SteeringBehavior::SeparationOn)
		 .def("SeparationOff",&SteeringBehavior::SeparationOff)
		 .def("WallAvoidanceOn",&SteeringBehavior::WallAvoidanceOn)
		 .property("m_vTarget",&SteeringBehavior::Target,&SteeringBehavior::SetTarget)
	 ];
}
void RegisterToLuaCActor_PathPlanner(lua_State* Lua){
	module(Lua)
	[
		 class_<CActor_PathPlanner>("CActor_PathPlanner")
		 .def("RequestPathToPosition",&CActor_PathPlanner::RequestPathToPosition)
	 ];
}

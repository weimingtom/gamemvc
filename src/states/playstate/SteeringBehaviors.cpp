...

{
	...
	m_vWanderTarget = Vector2D( m_dWanderRadius * cos( theta ), m_dWanderRadius
			* sin( theta ) );
...
Vector2D SteeringBehavior::Calculate() {
	...
	if ( On( separation ) ) {
		m_pWorld->TagVehiclesWithinViewRange( m_pCActorMapa, m_dViewDistance );
	}
...
Vector2D SteeringBehavior::Arrive(	const Vector2D &target,
	...
	if ( dist > 0 ) {
		...
		speed = MinOf( speed, m_pCActorMapa->MaxSpeed() );
...
void SteeringBehavior::CreateFeelers() {
	...
	m_Feelers[0] = m_pCActorMapa->Pos() + m_dWallDetectionFeelerLength
			* m_pCActorMapa->Heading() * m_pCActorMapa->Speed();

	//feeler to left
	Vector2D temp = m_pCActorMapa->Heading();
	Vec2DRotateAroundOrigin( temp, HalfPi * 3.5 );
	m_Feelers[1] = m_pCActorMapa->Pos() + m_dWallDetectionFeelerLength / 2.0
			* temp;

	//feeler to right
	temp = m_pCActorMapa->Heading();
	Vec2DRotateAroundOrigin( temp, HalfPi * 0.5 );
	m_Feelers[2] = m_pCActorMapa->Pos() + m_dWallDetectionFeelerLength / 2.0
			* temp;
}

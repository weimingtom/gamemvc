...
void PlayView::setMouse(const std::string& text, const int &X, const int &Y) {
...
void PlayView::moveZone(int X, int Y) {

	m_ZoneView->updateMoveView(X, Y);
...
void PlayView::moveView(int X, int Y) {

	m_ZoneView->moveView(X, Y);
...
PlayView::PlayZoneView::PlayZoneView(const PlayView& play) :
	m_play(play), m_zone(m_play.getXmlGui().getWidget("zone")), m_areaZone(
			m_zone->getChildrenArea()), m_scroller(new Scroller(
			m_play.getModel()->getMap(), m_play.getModel()->getResolution(),
			m_play.getModel()->getTopPadding(), m_areaZone)),
...
void PlayView::PlayZoneView::updateMoveView(int X, int Y) {
...
void PlayView::PlayZoneView::draw() {
	...
	//
	if (m_move)	moveView(moveX, moveY);
	//
	...
	//
	std::for_each(allPoints.begin(), allPoints.end(), boost::bind(
			&PlayView::PlayZoneView::PaintAllTerrain, this, _1));
	//
	// Dibujamos los Buildings.
	//
	std::for_each(allPoints.begin(), allPoints.end(), boost::bind(
			&PlayView::PlayZoneView::PaintAllBuilding, this, _1));
	//
	// Dibujamos los Resource.
	//
	std::for_each(allPoints.begin(), allPoints.end(), boost::bind(
			&PlayView::PlayZoneView::PaintAllResource, this, _1));
...
void PlayView::PlayZoneView::PaintAllTerrain(const gcn::Point& paintPoint) {
	...
	gcn::Point pScreen =
			m_scroller->LocalToScreen(pLocal.GetX(), pLocal.GetY());

	std::for_each(terrainCell.begin(), terrainCell.end(), boost::bind(
			&CTerrainMapa::Draw, _1, game.getGui().getGraphics(),
			pScreen.GetX(), pScreen.GetY()));
...
void PlayView::PlayZoneView::PaintAllBuilding(const gcn::Point& paintPoint) {
	...
	gcn::Point pScreen =
			m_scroller->LocalToScreen(pLocal.GetX(), pLocal.GetY());
	std::for_each(buildingCell.begin(), buildingCell.end(), boost::bind(
			&CBuildingMapa::Draw, _1, game.getGui().getGraphics(),
			pScreen.GetX(), pScreen.GetY()));
...
void PlayView::PlayZoneView::PaintAllResource(const gcn::Point& paintPoint) {
	...
	gcn::Point pScreen =
			m_scroller->LocalToScreen(pLocal.GetX(), pLocal.GetY());
	std::for_each(ResourceCell.begin(), ResourceCell.end(), boost::bind(
			&CResourceMapa::Draw, _1, game.getGui().getGraphics(),
			pScreen.GetX(), pScreen.GetY()));

}
void PlayView::PlayZoneView::moveView(int x, int y) {
...
PlayView::PlayMsgLeftView::PlayMsgLeftView(PlayView& play) :
	...
	m_posX = static_cast<gcn::TextField*> (m_play.getXmlGui().getWidget("X"));
	m_posY = static_cast<gcn::TextField*> (m_play.getXmlGui().getWidget("Y"));
...
void PlayView::PlayMsgCenterView::initialize() {
	...
	m_msgcenter->addMouseListener(
			m_play.getController()->getMsgCenterMouseListener());
...
void PlayView::PlayMsgCenterView::draw() {
	...
	game.getGui().getGraphics()->drawLine(10, 10, 100, 100);
	game.getGui().getGraphics()->drawText("Otro texto de prueba", 15, 15);
	game.getGui().getGraphics()->popClipArea();

...
Goal_MoveToPosition::Goal_MoveToPosition( 	CActorMapa* pBot,
	...
	Goal_Composite < CActorMapa > ( pBot, goal_move_to_position ),
			m_vDestination( pos ),
			m_iRetries( 0 ),
			m_iLastRetries( 0 ){
}
...
void Goal_MoveToPosition::Activate() {
	...
	if ( m_iRetries < maxRetries ) {
		...
		if (m_iLastRetries < maxLastRetries){
			...
			AddSubgoal( new Goal_Stopped(m_pOwner, 0.5));
...
/*
	...
	//	gdi->BlueBrush();
	gdi->CircleISO( m_vDestination, 6 );
	//	gdi->RedBrush();
	gdi->SetPenColor( CColor::Red );
	gdi->CircleISO( m_vDestination, 4 );
	//	gdi->YellowBrush();
